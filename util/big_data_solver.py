...
class GAN_Wrapper_Solver(object):
    ...
    def train(self,
        ...
        for epoch in epochs:  # for every epoch...
            ...
            train_minibatches = enumerate(train_minibatches)
            val_minibatches = enumerate(val_minibatches)
            for i in range(len(train_loader)):  # for every minibatch in training set
                # RUN GENERATOR steps times
                model.train()  # TRAINING mode (for dropout, batchnorm, etc.)
                all_output_images = []
                sample = next(train_minibatches)[1]
                ...
                if i != 0 and i % val_nth == 0:
                    ...
                    if len(val_loader) > 0:
                        ...
                        with torch.no_grad():
                            ...
                            for j in tqdm(range(val_len)):
                                ...
                                idx, sample = next(val_minibatches)

                                nvs_losses, output, nvs_accs = self.nvs_solver.forward_pass(model, sample)
                                val_loss, val_acc = self.log_iteration_loss_and_acc(nvs_losses, nvs_accs, 'Val/', epoch * len(val_loader) + idx)
                                val_losses.append(val_loss)
                                val_accs.append(val_acc)

                                # Print loss every log_nth iteration
                                if log_nth_iter != 0 and j % log_nth_iter == 0:
                                    print("[Iteration {cur}/{max}] Val loss: {loss}".format(cur=idx + 1,
                                                                                            max=len(val_loader),
                                                                                            loss=val_loss))
                                    self.nvs_solver.visualize_output(output, tag="val", step=epoch*len(val_loader) + idx, depth=self.log_depth)
                    ...
                    self.log_epoch_loss_and_acc(mean_train_loss, mean_val_loss, mean_train_acc, mean_val_acc, custom_epochs)
                    custom_epochs += 1
